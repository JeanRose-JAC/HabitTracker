...
@Composable
fun DeepScreen2(name: String?,modifier: Modifier = Modifier, authViewModel: AuthViewModel =
        ...
        LazyColumn(contentPadding = it){
            ...
            item{
                if(showList.value){
                    ...
                    Button(onClick = {
                        val resultIntent = activity.intent
                        resultIntent.putExtra("resultData", "$email") // Set the value to return as a result
                        localContext.setResult(Activity.RESULT_OK, resultIntent)
                        localContext.finish() // Finish the activity
                    },
...
fun tryDark() {
    HabitTrackerAppTheme() {
        com.example.habittrackerapp.signInSignUp.UserSignUp();
    }
}
